  ...
  return (
    <div>
      ...
      <section style={styles.flowDiagram}>
        ...
        <div style={styles.arrowContainer}><FilledArrowDown /></div>
            ...
            <div style={styles.sensorsGrid}>
              {sensors.map((sensor, i) => (
                ...
                >
                  ...
                  {hoveredSensor === i && (
                    <div style={styles.tooltip}>{sensor.info}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          {[
            ...
            <div key={index} className="benefit-card" style={styles.benefitCard}>
              <div style={{ ...styles.benefitIconWrapper, backgroundColor: `${item.color}15` }}>
                <div style={{ ...styles.benefitIcon, color: item.color }}>{item.icon}</div>
              </div>
        ...
        <div style={styles.stepsWrapper}>
          {[
            ...
            <React.Fragment key={index}>
              <div className="step-card clickable-step" style={styles.stepCard} onClick={item.onClick}>
...
const styles = {
  // Hero
  hero: { marginTop: '90px', height: '75vh', position: 'relative', overflow: 'hidden' },
  ...
  arrowContainer: { textAlign: 'center', margin: '0.6rem 0', display: 'flex', justifyContent: 'center', alignItems: 'center' },
  ...
  layer: { background: '#f8f9fa', borderRadius: '16px', padding: '1.5rem', border: '1px solid rgba(0,180,216,0.1)' },
  layerTitle: { fontSize: '1rem', fontWeight: 'bold', color: '#023047', marginBottom: '1rem', textAlign: 'center', paddingBottom: '0.5rem', borderBottom: '2px solid #00b4d8', display: 'block', width: 'fit-content', margin: '0 auto 1rem auto' },
  sensorsGrid: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.8rem', marginBottom: '1rem' },
  ...
  centerBox: { background: 'linear-gradient(135deg, #023047 0%, #0077b6 100%)', padding: '1.2rem', borderRadius: '12px', textAlign: 'center', color: 'white', transition: 'all 0.3s', margin: '0.5rem 0 0' },
...
styleSheet.textContent = `
  ...
  .feature-card:hover { transform: translateY(-5px); box-shadow: 0 12px 30px rgba(0,180,216,0.2); }
  .benefit-card:hover { transform: translateY(-5px); box-shadow: 0 12px 30px rgba(0,180,216,0.2); }
  .step-card:hover { transform: translateY(-3px); box-shadow: 0 8px 20px rgba(0,180,216,0.15); background: linear-gradient(135deg, #f8f9fa 0%, #e8f4f8 100%); }
  .sensor-box:hover { transform: translateY(-3px); box-shadow: 0 8px 15px rgba(0,180,216,0.2); border: 1px solid #00b4d8; background: linear-gradient(135deg, #ffffff 0%, #f0f9fb 100%); }
  .backend-box:hover { transform: translateY(-3px); box-shadow: 0 8px 15px rgba(0,180,216,0.2); border: 1px solid #00b4d8; }
  .frontend-box:hover { transform: translateY(-3px); box-shadow: 0 8px 15px rgba(0,180,216,0.2); border: 1px solid #00b4d8; }
  .center-box:hover { transform: scale(1.02); box-shadow: 0 10px 25px rgba(0,180,216,0.3); }
`;
